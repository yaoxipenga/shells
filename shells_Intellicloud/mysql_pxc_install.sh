...
#read master
master=`cat /root/shells/ip.txt | grep mysql_pxc | grep mysql_pxc1 | awk -F':' '{print $2}'`
#echo -n "请输入slave1节点的IP:"
#read slave1
slave1=`cat /root/shells/ip.txt | grep mysql_pxc | grep mysql_pxc2 | awk -F':' '{print $2}'`
#echo -n "请输入slave2节点的IP:"
#read slave2
slave2=`cat /root/shells/ip.txt | grep mysql_pxc | grep mysql_pxc3 | awk -F':' '{print $2}'`
#echo -n "请输入要设置的mysql的密码"
#read passwd
passwd=`cat /root/shells/ip.txt | grep mysql_pxc | grep password | awk -F'=' '{print $2}'`

#function choice(){
#	echo -n "请选择节点(1.master;2.slave1;3.slave2)"
#        read choice 
        if [[ ${IP_ADDRESS} == ${master} ]];then 
            echo "选择了master"
