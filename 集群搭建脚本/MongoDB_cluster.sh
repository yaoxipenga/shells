...
tar -zxvf ./mongodb-linux-x86_64-enterprise-rhel70-4.4.1.tgz
mv ./mongodb-linux-x86_64-enterprise-rhel70-4.4.1 /usr/local/mongodb
...
function choice(){
	echo -n "请选择节点(1.master;2.slaves;3.arbite)"
        read choice 
        if [[ ${choice} == "1" ]];then 
            echo "选择了master"
